...
function M.LuaUnit:protectedCall(classInstance, methodInstance, prettyFuncName)
    ...
    if classInstance then
        ...
        ok, _ = xpcall(function() methodInstance(classInstance) end, err_handler)
    else
        ok, _ = xpcall(function() methodInstance() end, err_handler)
    end
    if ok then
...
function isMoonSharp()
    return M._LUAVERSION == "MoonSharp 2.0.0.0"
end
